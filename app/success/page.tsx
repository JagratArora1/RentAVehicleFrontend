...
export default function SuccessPage() {
  const router = useRouter();
  const [parsedBookingData, setParsedBookingData] = useState<BookingData[] | null>(null);
  const [token, setToken] = useState<string | null>(null);

  useEffect(() => {
    const storedBookingData = localStorage.getItem("bookdata");
    if (storedBookingData) {
      setParsedBookingData(JSON.parse(storedBookingData));
    }
  ...
  useEffect(() => {
    const saveBooking = async () => {
      if (!parsedBookingData || !token) return;
      ...
      const bookingData = {
        ...
        endDate: parsedBookingData[0]?.endDate || "",
        totalPrice:
          typeof parsedBookingData[0]?.totalPrice === "number" ? parsedBookingData[0]?.totalPrice : 0,
        status: "confirmed",
      };
